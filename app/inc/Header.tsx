...
const Header = () => {
  ...
  return (
    <header className="flex flex-wrap border-b border-zinc-800/30 md:justify-start md:flex-nowrap z-50 w-full text-sm">
      <nav className="relative max-w-[85rem] w-full mx-2 py-3 px-4 md:flex md:items-center md:justify-between md:py-0 md:px-6 lg:px-8 xl:mx-auto" aria-label="Global">
        ...
        <div id="navbar-collapse-with-animation" className="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow md:block">
          <div className="flex flex-col gap-y-4 gap-x-0 mt-5 md:flex-row md:items-center md:justify-end md:gap-y-0 md:gap-x-7 md:mt-0 md:ps-7">
            <Link className={`font-medium ${pathName === '/' ? 'text-blue-600' : 'text-gray-500 hover:text-blue-600'} md:py-6`} href="/">anasayfa</Link>
            <Link className={`font-medium ${pathName === '/about' ? 'text-blue-600' : 'text-gray-500 hover:text-blue-600'} md:py-6`} href="/about">hakkımda</Link>
            <Link className={`font-medium ${pathName === '/contact' ? 'text-blue-600' : 'text-gray-500 hover:text-blue-600'} md:py-6`} href="/contact">iletişim</Link>
            ...
            <Link className={`flex items-center relative group gap-x-2 font-medium ${pathName === '/appointment' ? 'text-blue-600' : 'text-gray-500 hover:text-blue-600'} md:border-s md:border-zinc-950/20 md:my-6 md:ps-6`} href="/appointment">
              ...
              <span className='ml-5'>Randevu</span>
            </Link>
          </div>
        </div>
      </nav>
